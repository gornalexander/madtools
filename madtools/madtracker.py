...
def initialize_madx(input=None, call=None, stdout=False, redefine_elements=None, select_columns=True, **kwargs):
    madx = Madx(stdout=stdout)
    madx.option(echo=True, warn=True, info=False, debug=False, verbose=False)
    
    if call:
        if isinstance(call, list):
            for c in call:
                madx.call(c, chdir=True)
        elif isinstance(call, str):
            madx.call(call, chdir=True)
        else:
            raise ValueError('Call must be a string or a list of strings')
    ...
    if redefine_elements:
        redefine_elements(madx, **kwargs)
